...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if ([newSuperview isKindOfClass:[UIScrollView class]]) {
        ...
        self.mj_y = -self.mj_height - scrollView.contentInset.top;
        return;
    }
    // 设置自己的位置和尺寸
...
- (void)adjustStateWithContentOffset
{
    ...
    CGFloat refreshOffset = happenOffsetY - currentOffsetY;
    if (refreshOffset > 0) {
        CGFloat ratio = refreshOffset / self.mj_height;
        self.alpha = ratio > 0.02 ? ratio : 0.02;
    } else {
        self.alpha = 0.02;
    }
}
